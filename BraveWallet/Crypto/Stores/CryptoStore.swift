...
extension PendingRequest: Identifiable {
  var id: String {
    ...
    case let .addChain(chain): return "addChain-\(chain.id)"
    ...
    case let .signMessage(signRequest): return "signMessage-\(signRequest.id)"
    }
  }
}
...
public class CryptoStore: ObservableObject {
  ...
  func fetchPendingWebpageRequest() async -> PendingRequest? {
    ...
    if let chainRequest = await rpcService.pendingChainRequests().first {
      ...
      return .signMessage(signMessageRequest)
